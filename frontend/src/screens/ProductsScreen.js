...
const ProductsScreen = (props) =>{
    ...
    const deleteHandler = (product) =>{
     ...
     return <div className="content content-margined">
       ...
       <h3>Products</h3>
       <button  className="buttoncreate" onClick={()=>openModal({})}>
    ...
    <form onSubmit={submitHandler} >
      <ul className="form-container">
        ...
        <li>
          <button type="submit" className="buttonnohover">{id ? "Update" : "Create"}</button>
        </li>
        <li>
          <button type="button" onClick={() => setModalVisible(false)} className="buttonblack">Back</button>
        </li>
      </ul>
    </form>
    </div>
     ...
     <div className="product-list">
       <table className="table">
         ...
         <tbody>
           ...
           <td>{product.brand}</td>
           <th>
             <button className="button green" onClick={() => openModal(product)}>Edit</button>
             {' '}
             <button className="button red" onClick={() => deleteHandler(product)}>Delete</button>
           </th>
           </tr>))}
         </tbody>
       </table>
